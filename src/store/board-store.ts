    ...
    persist(
        (set, get) => ({
            ...
            getComments: (boardId: string) => {
                return get().boardData.get(boardId)?.comments || [];
            },

            setComments: (boardId: string, comments: BoardComment[]) => {
                set((state) => {
                    const data = state.boardData.get(boardId);
                    if (!data) return state;
                    const boardData = new Map(state.boardData);
            ...
            addComment: (boardId: string, comment: BoardComment) => {
                set((state) => {
                    const data = state.boardData.get(boardId);
                    if (!data) return state;
                    const boardData = new Map(state.boardData);
            ...
            replaceComments: (boardId: string, comments: BoardComment[]) => {
                set((state) => {
                    const data = state.boardData.get(boardId);
                    if (!data) return state;
                    const boardData = new Map(state.boardData);
            ...
            moveBoard: (boardId: string, targetWorkstreamId: string) => {
                set((state) => {
                    ...
                    if (!board || board.workstreamId === targetWorkstreamId)
                        return state;
                    ...
                    if (oldWorkstream) {
                        workstreams.set(board.workstreamId, {
                            ...oldWorkstream,
                            boardIds: oldWorkstream.boardIds.filter(
                                (id) => id !== boardId,
                            ),
                            metadata: {
            ...
            setDiskStorageEnabled: (
                ...
                set((state) => ({
                    settings: {
                        ...
                        diskStorageDirectoryName: enabled
                            ? directoryName
                            : undefined,
                    },
                }));

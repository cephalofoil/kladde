...
export interface SearchResult {
  ...
  matchType: "board-name" | "tile-title" | "tile-content" | "text-element" | "frame-label";
...
function getElementSearchableContent(element: BoardElement): {
  ...
  switch (element.type) {
    ...
    case "tile":
      ...
      if (tileContent?.bookmarkTitle) parts.push(tileContent.bookmarkTitle);
  ...
  for (const [boardId, board] of boards) {
    ...
    if (data?.elements) {
      for (const element of data.elements) {
        ...
        // Check content match
        else if (searchable.content?.toLowerCase().includes(lowerQuery)) {
          results.push({
            boardId,
            boardName: board.name,
            workstreamId: board.workstreamId,

...
export class CollaborationManager {
  ...
  private handleBeforeUnload = () => {
    try {
      ...
      console.warn("[Collaboration] Failed to clear local awareness:", error);
    }
  ...
  async setEncryptionKey(key: CryptoKey): Promise<void> {
    ...
    const hasUnencryptedElements = stored.some((el) => !isEncryptedElement(el));
  ...
  async setElements(nextElements: BoardElement[]): Promise<void> {
    ...
    if (key) {
      try {
        ...
        console.error("[Collaboration] Failed to encrypt elements:", error);
  ...
  async addElement(element: BoardElement): Promise<void> {
    ...
    if (this.encryptionKey) {
      try {
        ...
        console.error("[Collaboration] Failed to encrypt element:", error);
  ...
  async updateElement(
    ...
    if (index !== -1) {
      ...
      try {
        ...
        console.error("[Collaboration] Failed to update element:", error);
      }
    }
  }
   ...
   */
  onElementsChange(
    callback: (elements: BoardElement[], info?: { isRemote: boolean }) => void,
  ): () => void {

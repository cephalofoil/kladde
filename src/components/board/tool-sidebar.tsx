...
export function ToolSidebar({
  ...
  const arrowEndOptions = useMemo(
    () =>
      [
        ...
        { id: "triangle_outline", label: "Triangle Outline" },
        { id: "diamond", label: "Diamond" },
        { id: "diamond_outline", label: "Diamond Outline" },
        { id: "crowfoot_one", label: "Crowfoot One" },
        { id: "crowfoot_many", label: "Crowfoot Many" },
        { id: "crowfoot_one_or_many", label: "Crowfoot One or Many" },
      ] as Array<{
  ...
  const fillPatternLabel =
    ...
    fillPattern;
  const hasFillColor =
    !!fillColor && fillColor !== "none" && fillColor !== "transparent";
  const renderFillPatternIcon = (
    pattern: NonNullable<BoardElement["fillPattern"]>,
  ) => {
    const patternIconMap: Record<string, string> = {
      solid: "/icons/fillpattern.svg",
      hachure: "/icons/hachurepattern.svg",
      "cross-hatch": "/icons/crosshatchpattern.svg",
      zigzag: "/icons/zigzagpattern.svg",
    };
  ...
  const renderArrowEndPreview = (
    ...
    const tip = { x: 22, y: cy };
    const from = { x: 8, y: cy };
    const normalized = normalizeArrowhead(endType);
    const points = getArrowheadPoints(tip, from, normalized, sw);

    const renderMarker = () => {
      if (normalized === "none" || !points) return null;
      ...
      if (
        ...
        return (
          <circle
            ...
            strokeWidth={normalized === "circle_outline" ? sw : 0}
          />
        );
      }
      ...
      if (normalized === "triangle" || normalized === "triangle_outline") {
        ...
        return (
          <polygon
            ...
            strokeWidth={normalized === "triangle_outline" ? sw : 0}
      ...
      if (normalized === "diamond" || normalized === "diamond_outline") {
        ...
        return (
          <polygon
            ...
            strokeWidth={normalized === "diamond_outline" ? sw : 0}
            strokeLinejoin="round"
          />
        );
      }
  ...
  // Show vertical text alignment for shapes that can contain text
  const showShapeTextAlign = hasSelectedElements
    ? selectedElements.some(
        (el) =>
          (el.type === "rectangle" ||
            el.type === "diamond" ||
  ...
  const isFrameOnlySelection =
    hasSelectedElements && selectedElements.every((el) => el.type === "frame");
  ...
  const optionsControls = (
    <>
          ...
          >
            ...
            <button
              ...
              className={cn(
                ...
                showFontSubmenu || !FONTS.some((f) => f.value === fontFamily)
      ...
      {/* Vertical text alignment for shapes with text */}
      {showShapeTextAlign && onTextVerticalAlignChange && (
        <div className="space-y-2">
          <label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">
            Vertical Align
          </label>
          <ToggleGroup
            ...
            onValueChange={(value) => {
              if (!value) return;
              onTextVerticalAlignChange(value as "top" | "middle" | "bottom");
            }}
            variant="outline"
            size="sm"
            className="flex gap-1"
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-3 gap-1">
            {FRAME_STYLE_OPTIONS.map((styleOption) => {
              const isActive = uniformFrameStyle === styleOption.id;
              return (
                <button
                  key={styleOption.id}
                  type="button"
                  onClick={() => onFrameStyleChange(styleOption.id)}
                  className={cn(
                    CONTROL_BUTTON,
                    "h-auto py-2 px-1 flex flex-col items-center gap-1",
                    isActive ? CONTROL_BUTTON_SELECTED : undefined,
                  )}
                ...
                >
                  <div
                    className={cn(
                      ...
                      isActive ? "border-foreground/30" : "border-border/60",
                    )}
                  >
      ...
      {showCornerRadius && (
        <div className="space-y-2">
          ...
          <ToggleGroup
            type="single"
            value={cornerRadius > 0 ? "rounded" : "sharp"}
            onValueChange={(value) => {
              if (!value) return;
              onCornerRadiusChange(value === "rounded" ? 25 : 0);
            }}
            variant="outline"
            size="sm"
            className="flex gap-1"
          >
            ...
            >
              ...
              >
                <path
                  d="M4 12V7C4 5.34315 5.34315 4 7 4H12"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />
              </svg>
      ...
      <div className="bg-card/95 backdrop-blur-md border border-border/60 dark:border-transparent rounded-md shadow-2xl py-2 px-1.5 flex flex-col items-center gap-2 max-h-[calc(100vh-160px)] overflow-y-auto overscroll-contain scrollbar-thin scrollbar-thumb-border scrollbar-track-transparent text-foreground">
        ...
        {hasSelectedElements && onToggleLockSelected && (
          <button
            ...
            className={cn(iconButton, isSelectionLocked && "text-amber-500")}
        ...
        <div
          ...
          style={{
            ...
            top: fontSubmenuButtonRef.current
              ? fontSubmenuButtonRef.current.getBoundingClientRect().top - 8
              : 200,
          }}
        >
          ...
          >
            <div className="grid grid-cols-2 gap-1">
              {arrowEndOptions.map((opt) => {
                const currentValue =
                  openArrowEndMenu === "start"
                    ? normalizeArrowhead(arrowStart)
                    : normalizeArrowhead(arrowEnd);
                const isSelected = currentValue === normalizeArrowhead(opt.id);
                return (
                  <button

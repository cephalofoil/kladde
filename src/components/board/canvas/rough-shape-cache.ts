      ...
      case "arrow": {
        ...
        if (hasCorner && control) {
          if (connectorStyle === "curved") {
            ...
            // Expand elbow control points into full polyline
            const route =
              element.elbowRoute ||
            ...
            let elbowPolyline: Point[];
            if (element.points.length === 3) {
              // Simple 3-point elbow: expand based on route
              elbowPolyline =
                route === "vertical"
                  ? [
                      start,
              ...
              // Multi-point elbow: expand using helper
              elbowPolyline = getElbowPolylineForVertices(element.points, 0.5);
            }
        ...
        if (element.type === "arrow") {
          ...
          const markerSize = Math.max(6, (element.strokeWidth ?? 2) * 3);

          // Use pathData for arrowhead positioning (important for elbow connectors)
          const renderPoints: Point[] = pathData.map(([x, y]) => ({ x, y }));
  ...
  switch (markerType) {
    ...
    case "diamond-outline": {
      ...
      const diamondPoints: [number, number][] = [
        ...
        [tip.x + size * Math.cos(angle), tip.y + size * Math.sin(angle)],
        [
          tip.x - halfSize * Math.cos(perpAngle),
          tip.y - halfSize * Math.sin(perpAngle),
        ],

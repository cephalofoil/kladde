    ...
    (e: ReactMouseEvent) => {
      ...
      if (frameLabelTarget) {
        ...
        const frameElement = frameId
          ? elements.find((el) => el.id === frameId && el.type === "frame")
          : null;
        if (frameElement) {
      ...
      if (tool === "select") {
        ...
        if (selectedBounds && selectedIds.length >= 1) {
          ...
          const rotationDegForHandles =
            selectedIds.length === 1
              ? (elements.find((el) => el.id === selectedIds[0])?.rotation ?? 0)
              : 0;
  ...
  const handleMouseUp = useCallback(() => {
    ...
    if (isLassoSelecting) {
      if (lassoPoints.length >= 3) {
        const lassoSelected = getLassoSelectedIds(elements, lassoPoints).filter(

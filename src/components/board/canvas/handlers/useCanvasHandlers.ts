...
interface UseCanvasHandlersProps {
  ...
  lineHeight: number;
  fillPattern: NonNullable<BoardElement["fillPattern"]>;
...
export function useCanvasHandlers({
  ...
  const { pan, setPan, zoom, isPanning, setIsPanning, panStart, setPanStart } =
    viewport;
  const {
    textInput,
    setTextInput,
  ...
  const connectorUpdateRafRef = useRef<number | null>(null);
  const pendingConnectorUpdateRef = useRef<{
    id: string;
    updates: Partial<BoardElement>;
  } | null>(null);

  // Helper to schedule RAF-throttled connector point updates
  const scheduleConnectorUpdate = useCallback(
    (id: string, updates: Partial<BoardElement>) => {
      pendingConnectorUpdateRef.current = { id, updates };
      if (connectorUpdateRafRef.current === null) {
        connectorUpdateRafRef.current = requestAnimationFrame(() => {
          connectorUpdateRafRef.current = null;
          const pending = pendingConnectorUpdateRef.current;
          if (!pending) return;
          pendingConnectorUpdateRef.current = null;
          onUpdateElement(pending.id, pending.updates);
        });
      }
    },
    [onUpdateElement],
  ...
  const throttledArrowUpdates = useMemo(() => getConnectedArrowUpdates, []);
  ...
  const getElementsToErase = useCallback(
    (point: Point): string[] => {
      ...
      currentElements.forEach((el) => {
        ...
        if (allowRemoteSelectionLock && remotelySelectedIds.has(el.id)) return;
        if (el.type === "pen" || el.type === "line" || el.type === "arrow") {
          const getConnectorHitTestPoints = (): Point[] => {
            ...
            if (style === "elbow") {
              ...
              const end = el.points[el.points.length - 1] ?? start;
              ...
              const points =
                el.points.length === 3
                  ...
                  : getElbowPolylineForVertices(el.points, elbowEps);
              return points;
            }
            ...
            if (style === "curved") {
              ...
              const sampleCatmullRom = (
                ...
                for (let i = 0; i < points.length - 1; i++) {
                  ...
                  for (let s = startStep; s <= stepsPerSegment; s++) {
                    const t = s / stepsPerSegment;
                    out.push(getCubicBezierPoint(p1, c1, c2, p2, t));
                  }
                }
                return out;
              };
          ...
          const pointsToCheck =
            el.type === "pen" ? el.points : getConnectorHitTestPoints();
  ...
  const handleMouseMove = useCallback(
    (e: ReactMouseEvent) => {
      ...
      if (isLassoSelecting) {
        ...
        if (nextPoints.length >= 3) {
          const lassoSelected = getLassoSelectedIds(
            ...
            (id) => !allowRemoteSelectionLock || !remotelySelectedIds.has(id),
          );
      ...
      if (isRotating && rotateStart) {
        ...
        const arrowUpdates =
          throttledArrowUpdates([rotateStart.elementId], tempElements) ?? [];
      ...
      if (
        ...
        if (edgeHandle) {
          ...
          const handlePos = rotationDeg
            ...
            : localHandle;
          if (
            Math.hypot(point.x - handlePos.x, point.y - handlePos.y) <=
      ...
      if (draggingConnectorPoint && originalElements.length === 1) {
        ...
        if (
          ...
          if (
            ...
            scheduleConnectorUpdate(originalElement.id, {
              points: newPoints,
            });
            return;
          }
          ...
          if (
            ...
            scheduleConnectorUpdate(originalElement.id, updates);
            return;
          }
          ...
          if (
            ...
            if (isEndpoint) {
              ...
              if (snapResult) {
                ...
                if (connectorStyle === "sharp" && snapResult.outOfLineOfSight) {
          ...
          scheduleConnectorUpdate(originalElement.id, { points: newPoints });
        }
        return;
      }
      ...
      if (isDragging && dragStart && originalElements.length > 0) {
        ...
        // Calculate alignment guides for snapping
        // Only for non-pen elements and when we have shape-like elements
        ...
        if (hasShapeElements) {
          ...
          // Also exclude arrows that are connected to the dragged elements
          // to prevent snapping to arrows attached to the object being moved
          const draggedIdSet = new Set(draggedIds);
          for (const el of currentElements) {
            if (el.type === "arrow" || el.type === "line") {
              const startConnectedId = el.startConnection?.elementId;
              const endConnectedId = el.endConnection?.elementId;
              if (
                (startConnectedId && draggedIdSet.has(startConnectedId)) ||
                (endConnectedId && draggedIdSet.has(endConnectedId))
              ) {
        ...
        if (movedShapeIds.length > 0) {
          ...
          const movedIds = new Set(originalElements.map((el) => el.id));
      ...
      if (
        ...
        const updateResizeAlignmentGuides = (
          ...
          for (const el of currentElements) {
            if (el.type === "arrow" || el.type === "line") {
              ...
              const endConnectedId = el.endConnection?.elementId;
              if (
                (startConnectedId && resizedIdSet.has(startConnectedId)) ||
                (endConnectedId && resizedIdSet.has(endConnectedId))
              ) {
                excludeIds.add(el.id);
              }
            }
        ...
        const applyResizeUpdates = (
          ...
          const batchUpdates = [...baseUpdates];

          if (movedShapeIds.length > 0) {
            const tempElements = currentElements.map((el) => {
              const update = baseUpdates.find((u) => u.id === el.id);
              if (update) {
                return { ...el, ...update.updates };
              }
              return el;
            ...
            const arrowUpdates =
              throttledArrowUpdates(movedShapeIds, tempElements) ?? [];
            const movedIds = new Set(baseUpdates.map((u) => u.id));
            arrowUpdates.forEach((au) => {
              if (!movedIds.has(au.id)) {
                batchUpdates.push(au);
              }
          ...
          const minAbsHeight =
            originalElement?.type === "text"
              ? Math.max(
                  ...
                  (originalElement.fontSize ??
                    originalElement.strokeWidth * 4 + 12) *
                    (originalElement.lineHeight ?? 1.25),
                )
              : 0;
        ...
        if (selectedIds.length === 1 && originalElements.length === 1) {
          ...
          if (originalElement.type === "text") {
            ...
            const elLetterSpacing = originalElement.letterSpacing ?? 0;
            ...
            const lh = originalElement.lineHeight ?? 1.25;
        ...
        if (selectedIds.length === 1 && originalElements.length === 1) {
          const originalElement = originalElements[0];
          const shouldUpdateConnectedArrows =
            originalElement.type !== "pen" &&
            originalElement.type !== "line" &&
            originalElement.type !== "arrow" &&
            originalElement.type !== "laser";
          let elementUpdates: Partial<BoardElement> | null = null;
          ...
          if (originalElement.type === "text") {
            ...
            minAbsHeight = Math.max(
              2,
              fontSizeForMin! * (originalElement.lineHeight ?? 1.25),
            );
          } else if (
            originalElement.type === "tile" &&
          ...
          if (
            ...
            const alignmentResult = updateResizeAlignmentGuides(
              getAlignmentBoundsForResize(originalElement, textBounds),
              [selectedIds[0]],
            );
            const snappedBounds = applyResizeSnap(
              textBounds,
          ...
          if (elementUpdates) {
            applyResizeUpdates(
              [{ id: selectedIds[0], updates: elementUpdates }],
              shouldUpdateConnectedArrows ? [selectedIds[0]] : [],
            );
          }
          return;
        }
        ...
        applyResizeUpdates(batchUpdates, movedShapeIds);
        return;
      }
      ...
      switch (tool) {
        ...
        case "arrow": {
          ...
          if (snapResult) {
            ...
            if (activeConnectorStyle === "sharp" && startSnapTarget) {
  ...
  const handleMouseDown = useCallback(
    (e: ReactMouseEvent) => {
      ...
      if (frameLabelTarget) {
        e.preventDefault();
        const frameId = frameLabelTarget.getAttribute("data-element-id");
      ...
      if (frameHandleTarget && selectableClickedElement?.type === "frame") {
        ...
        const containedElements = currentElements.filter(
          (el) =>
            ...
            !el.hidden &&
            !el.locked &&
            (!allowRemoteSelectionLock || !remotelySelectedIds.has(el.id)),
        );
        const dragElements = [selectableClickedElement, ...containedElements];
        onStartTransform?.();
      ...
      if (isInteractive) {
        ...
        if (selectableClickedElement) {
          setSelectedIds(
            getGroupSelectionIds(selectableClickedElement, currentElements),
          );
        }
        return;
      }
      ...
      if (tool === "select") {
        ...
        if (
          ...
          const editLineHeight = selectableClickedElement.lineHeight ?? 1.25;
          const isTextBox = selectableClickedElement.isTextBox ?? true;
          setTextInput({
        ...
        // Double-click shape to edit text inside it
        // Works when clicking directly on the shape OR anywhere in selection bounds if shape is selected
        // OR when clicking inside any shape's bounds (for shapes with text)
        if (e.detail === 2 && !isRemotelySelected) {
          let shapeToEdit: BoardElement | null = null;

          // Check if we clicked directly on a shape
          if (
            selectableClickedElement &&
            (selectableClickedElement.type === "rectangle" ||
              selectableClickedElement.type === "diamond" ||
              selectableClickedElement.type === "ellipse")
          ) {
            shapeToEdit = selectableClickedElement;
          }
          // Or check if we have a single shape selected and clicked within its bounds
          else if (selectedIds.length === 1 && selectedBounds) {
            const selectedElement = currentElements.find(
              (el) => el.id === selectedIds[0],
            );
            if (
              ...
              if (pointInBounds) {
                shapeToEdit = selectedElement;
              }
            }
          }
          // Or check if we clicked inside any shape's bounds (useful for stroke-only shapes with text)
          if (!shapeToEdit) {
            // Find shapes that contain the click point, sorted by zIndex (topmost first)
            const shapesAtPoint = currentElements
              .filter(
                (el) =>
                  (el.type === "rectangle" ||
                    el.type === "diamond" ||
          ...
          if (shapeToEdit) {
            ...
            const editFontSize =
              shapeToEdit.fontSize ?? shapeToEdit.strokeWidth * 4 + 12;
            const editLineHeight = shapeToEdit.lineHeight ?? 1.25;
            setTextInput({
              x: shapeToEdit.x ?? 0,
              y: shapeToEdit.y ?? 0,
              width: shapeToEdit.width,
              height: shapeToEdit.height,
              isTextBox: true,
            });
            setTextValue(shapeToEdit.text ?? "");
            setEditingShapeTextId(shapeToEdit.id);
            setEditingTextStyle({
              strokeColor: shapeToEdit.strokeColor,
              strokeWidth: shapeToEdit.strokeWidth,
              opacity: shapeToEdit.opacity ?? 100,
              fontFamily: shapeToEdit.fontFamily || "var(--font-inter)",
              textAlign: shapeToEdit.textAlign || "center",
              fontSize: shapeToEdit.fontSize ?? editFontSize,
              letterSpacing: shapeToEdit.letterSpacing ?? 0,
              lineHeight: shapeToEdit.lineHeight ?? editLineHeight,
            });
            setSelectedIds([shapeToEdit.id]);
            setTimeout(() => {
              const editor = textInputRef.current;
              editor?.focus();
              if (editor) {
        ...
        if (selectedBounds && selectedIds.length >= 1) {
          ...
          const rotationDegForHandles =
            selectedIds.length === 1
              ? (currentElements.find((el) => el.id === selectedIds[0])
                  ?.rotation ?? 0)
              : 0;
          ...
          if (
            ...
            setIsDragging(true);
            setDragStart(point);
            setOriginalElements(dragSelectionElements.map((el) => ({ ...el })));
            return;
          }
        }
        ...
        if (selectableClickedElement) {
          ...
          const dragClickedElements = currentElements.filter(
            (el) => clickedIds.includes(el.id) && el.type !== "frame",
          );
          ...
          if (shiftPressed) {
            if (clickedAllSelected) {
              ...
              setSelectedIds([
                ...selectedIds,
                ...clickedIds.filter((id) => !selectedIds.includes(id)),
              ]);
            }
          } else {
            ...
            if (clickedAllSelected) {
              ...
              setIsDragging(true);
              setDragStart(point);
              setOriginalElements(dragClickedElements.map((el) => ({ ...el })));
            }
          }
        } else {
          // Start box selection
      ...
      if (tool === "text") {
        ...
        if (
          ...
          const editLineHeight = clickedElement.lineHeight ?? 1.25;
          const textBoxPadding = 8; // 4px on each side
          const isTextBox = clickedElement.isTextBox ?? true;
          const minHeight = isTextBox
            ? editFontSize * editLineHeight + textBoxPadding
        ...
        // Start tracking for click vs drag to create text.
      ...
      const newElement: BoardElement = {
        id: uuid(),
        type: elementType,
        points: [startSnapResult ? startSnapResult.snapPoint.point : point],
        strokeColor,
        strokeWidth: tool === "frame" ? 2 : strokeWidth,
        opacity,
        strokeStyle:
          tool === "frame" ? "solid" : isHighlighter ? "solid" : strokeStyle,
        lineCap,
        cornerRadius,
  ...
  const handleMouseUp = useCallback(() => {
    ...
    if (draggingConnectorPoint) {
      // Flush any pending RAF connector updates before ending the drag
      if (connectorUpdateRafRef.current !== null) {
        cancelAnimationFrame(connectorUpdateRafRef.current);
        connectorUpdateRafRef.current = null;
      }
      const pendingConnector = pendingConnectorUpdateRef.current;
      if (pendingConnector) {
        pendingConnectorUpdateRef.current = null;
        onUpdateElement(pendingConnector.id, pendingConnector.updates);
      }
      ...
      if (
        ...
        if (orig.type === "line" || orig.type === "arrow") {
          const index = draggingConnectorPoint.index;
          const isEndpoint = index === 0 || index === orig.points.length - 1;

          if (isEndpoint) {
            const otherEndpointIndex = index === 0 ? orig.points.length - 1 : 0;
      ...
      if (originalElements.length === 1) {
        ...
        if (
          ...
          const current = currentElements.find((el) => el.id === orig.id);
          if (current?.points && current.points.length >= 2) {
            ...
            const hasDiagonal = current.points.some((p, i) => {
              ...
              return (
                Math.abs(p.x - prev.x) > eps && Math.abs(p.y - prev.y) > eps
              );
            });
            if (!hasDiagonal) {
    ...
    if (tool === "text" && isDrawing && startPoint) {
      ...
      if (dragDistance >= 5) {
        ...
        const textBoxPadding = 8; // 4px on each side
        const minHeight = fontSize * lineHeight + textBoxPadding;
    ...
    if (currentElement && isDrawing) {
      ...
      if (currentElement.type === "pen" && currentElement.points.length >= 1) {
        ...
        const shouldFill =
          isClosed && endpointsInArea && currentElement.fillPattern !== "none";
        const finalElement: BoardElement = {
          ...currentElement,
          isClosed,
          fillPattern: shouldFill
  ...
  const handleTextSubmit = useCallback(
    (options?: { skipToolChange?: boolean; skipSelect?: boolean }) => {
      // Handle shape text editing (rectangle, diamond, ellipse)
      if (editingShapeTextId) {
        ...
        // Update the shape with text properties
        onUpdateElement(editingShapeTextId, {
        ...
        if (!options?.skipSelect) {
          setSelectedIds([editingShapeTextId]);
        }
      ...
      if (textInput && textValue.trim()) {
        ...
        const measuredHeight = textInputRef.current
          ...
          : undefined;
        const textBoxHeight = textInputRef.current
          ? measureWrappedTextHeightPx({
              text: textValue,
        ...
        const unboundedWidth = isTextBox
          ? (textInput.width ?? measuredWidth ?? unboundedSize.width)
          : (measuredWidth ?? unboundedSize.width);
        const unboundedHeight = isTextBox

...
interface RenderElementOptions {
  ...
  isRemotelyEditing: boolean;
}

interface MemoizedElementRendererProps extends RenderElementOptions {
...
const MemoizedElementRenderer = memo(
  ...
  (prev, next) =>
    ...
    prev.isMarkedForDeletion === next.isMarkedForDeletion &&
    prev.isTextEditing === next.isTextEditing &&
    prev.isRemotelyEditing === next.isRemotelyEditing &&
    prev.dragPreviewKey === next.dragPreviewKey &&
    prev.zoom === next.zoom &&
    prev.handDrawnMode === next.handDrawnMode &&
...
export function useCanvasRenderers({
  ...
  const adaptColor = useCallback(
    (color: string | undefined): string | undefined => {
      if (!color || !viewerTheme) return color;
      return adaptColorForTheme(color, viewerTheme);
    },
    [viewerTheme],
  );
  ...
  const getConnectorDragPreviewElement = useCallback(
    (element: BoardElement): BoardElement => {
      ...
      if (originalElement.type !== "line" && originalElement.type !== "arrow")
        return element;
      ...
      if (
        ...
        const isEndpoint = index === 0 || index === originalPoints.length - 1;
        ...
        const hasExistingConnection =
          originalElement.startConnection || originalElement.endConnection;
      ...
      if (
        ...
        const delta =
          axis === "x" ? localPoint.x - current.x : localPoint.y - current.y;
      ...
      if (
        ...
        if (snapTarget) {
          const otherEndpointIndex = isStart ? originalPoints.length - 1 : 0;
          const otherEndpoint = originalPoints[otherEndpointIndex];
          // Get existing connection on the other end
          ...
          // Reverse if dragging start point
          const finalPoints = isStart ? routedPoints.reverse() : routedPoints;
  ...
  const renderElementInner = (
    ...
    const baseElement = !isPreview
      ? getConnectorDragPreviewElement(element)
      : element;

    // Adapt colors for cross-theme visibility in collaboration
    // This ensures black strokes are visible in dark mode and vice versa
    const effectiveElement: BoardElement = viewerTheme
      ? {
          ...baseElement,
          strokeColor:
            adaptColor(baseElement.strokeColor) ?? baseElement.strokeColor,
          fillColor: adaptColor(baseElement.fillColor),
        }
      : baseElement;
    ...
    switch (effectiveElement.type) {
      case "pen": {
        ...
        const isHighlighter = effectiveElement.penMode === "highlighter";
            ...
            return (
              <g key={effectiveElement.id}>
                ...
                {pathD ? (
                  ...
                  <polyline
                    data-element-id={effectiveElement.id}
                    points={(polyPoints || effectiveElement.points)
                      .map((p) => `${p.x},${p.y}`)
                      .join(" ")}
                    fill="none"
                    stroke="transparent"
      ...
      case "text": {
        ...
        const isAtMinWidth =
          element.width !== undefined && element.width <= minCharWidth + 1;
      ...
      case "frame": {
        ...
        // Treat 'transparent' same as 'none' for hit detection - invisible fills shouldn't be clickable
        const hasVisibleFill =
          styledFillColor !== "none" && styledFillColor !== "transparent";
        // Convert transparent to none for proper pointer-events behavior in SVG
        const fillValue =
          styledFillColor === "transparent" ? "none" : styledFillColor;
        // Only visible parts should be clickable: if has fill AND stroke, allow both; if only stroke, only stroke; if only fill, only fill
        const pointerEventsValue =
          hasVisibleFill && element.strokeWidth > 0
            ? "visible"
        ...
        const overlayHeight = Math.max(frameHeight - overlayInset * 2, 0);
        ...
        const gridColumns = Math.max(0, Math.floor(overlayWidth / gridSize));
        const gridRows = Math.max(0, Math.floor(overlayHeight / gridSize));
                ...
                <defs>
                  ...
                  <clipPath id={overlayClipId}>
                    <rect
                      ...
                      rx={Math.max(frameCornerRadius - overlayInset, 0)}
                    />
                  </clipPath>
                </defs>
              )}
              ...
              overlayHeight > 0 && (
                <>
                  ...
                  <rect
                    ...
                    y={overlayY}
                    width={overlayWidth}
                    height={Math.min(measurementBarSize, overlayHeight)}
                    fill={barColor}
                    opacity={0.75}
                    clipPath={`url(#${overlayClipId})`}
                    pointerEvents="none"
                  ...
                  <rect
                    x={
                      overlayX + Math.max(overlayWidth - measurementBarSize, 0)
                    }
                    y={overlayY}
                    width={Math.min(measurementBarSize, overlayWidth)}
                    height={overlayHeight}
                    fill={barColor}
                    opacity={0.75}
                    clipPath={`url(#${overlayClipId})`}
                  ...
                  {Array.from({
                    ...
                    return (
                      <line
                        ...
                        y1={overlayY + 2}
                        x2={x}
                        y2={overlayY + Math.min(tickLength, overlayHeight)}
                        stroke={gridColor}
                        strokeWidth={1}
                        opacity={0.7}
                        pointerEvents="none"
                  ...
                  {Array.from({
                    ...
                    const x = overlayX + i * gridSize;
                    if (x > overlayX + overlayWidth - measurementBarSize) {
                      return null;
                    }
                    return (
                      <text
                        ...
                        y={
                          overlayY +
                          Math.min(measurementBarSize - 4, overlayHeight)
                        }
                        fill={gridColor}
                        fontSize={8}
                        fontWeight="700"
                    ...
                    return (
                      <line
                        ...
                        x1={
                          overlayX +
                          overlayWidth -
                          Math.min(tickLength, overlayWidth)
                        }
                        y1={y}
                  ...
                  {Array.from({
                    ...
                    const y = overlayY + i * gridSize;
                    if (y > overlayY + overlayHeight - measurementBarSize) {
                      return null;
                    }
                    return (
                      <text
                        ...
                        x={
                          overlayX +
                          overlayWidth -
                          Math.min(measurementBarSize - 4, overlayWidth)
                        }
                        y={y}
                        fill={gridColor}
                        fontSize={8}
              ...
              overlayHeight > 0 && (
                <>
                  <rect
                    ...
                    width={Math.min(24, overlayWidth * 0.18)}
                    height={overlayHeight}
                    fill="#1e5a5a"
                    opacity={0.65}
                    clipPath={`url(#${overlayClipId})`}
                  ...
                  {Array.from({ length: 6 }).map((_, idx) => (
                    <line
                      ...
                      x1={
                        frameX +
                        overlayInset +
                        Math.min(24, overlayWidth * 0.18) / 2
                      }
                      y1={
                        frameY + overlayInset + (idx + 1) * (overlayHeight / 7)
                      }
                      x2={
                        frameX +
                        overlayInset +
                  ...
                  <rect
                    ...
                    y={frameY + overlayInset + overlayHeight * 0.3}
                    width={Math.min(24, overlayWidth * 0.2)}
                    height={Math.min(36, overlayHeight * 0.2)}
                    fill="#e8dcc8"
                    opacity={0.9}
                    clipPath={`url(#${overlayClipId})`}
                    pointerEvents="none"
        ...
        return (
          ...
          >
            ...
            <path
              ...
              fill="#ef4444"
              opacity={
                isMarkedForDeletion ? elOpacity * 0.8 * 0.3 : elOpacity * 0.8
              }
  ...
  const renderConnectorControls = useCallback(
    (element: BoardElement) => {
      ...
      if (style === "elbow") {
        ...
        const startElbowMoveEdge = (e: ReactMouseEvent, segIndex: number) => {
          e.stopPropagation();
          onStartTransform?.();
        ...
        const renderInsertForRun = (run: {
          ...
          const canShow =
            segLenPx >= existingDiameterPx + insertDiameterPx + minGapPx;
          if (!canShow) return null;

          const cx = (a.x + b.x) / 2;
          const cy = (a.y + b.y) / 2;
          const dStart =
            Math.hypot(cx - elbowPolyline[0].x, cy - elbowPolyline[0].y) * zoom;
          const dEnd =
            Math.hypot(
              cx - elbowPolyline[elbowPolyline.length - 1].x,
              cy - elbowPolyline[elbowPolyline.length - 1].y,
            ) * zoom;
          if (dStart < minDistFromEndpointPx || dEnd < minDistFromEndpointPx)
            return null;
          ...
          const overlapsInsert = insertCenters.some(
            (p) => Math.hypot(cx - p.x, cy - p.y) * zoom < insertDiameterPx + 2,
          );
          ...
          return (
            <circle
              ...
              style={{
                cursor: run.orientation === "h" ? "ns-resize" : "ew-resize",
              }}
        ...
        const activeEdgeCircle =
          draggingConnectorPoint?.kind === "elbowEdge" &&
          draggingConnectorPoint?.range &&
          draggingConnectorPoint.edgeKey?.startsWith(`${element.id}-elbow-`)
            ? (() => {
      ...
      const startDrag = (
        ...
        if (
          ...
          const collinearHorizontal =
            Math.abs(prev.y - p.y) <= eps && Math.abs(next.y - p.y) <= eps;
          const collinearVertical =
            Math.abs(prev.x - p.x) <= eps && Math.abs(next.x - p.x) <= eps;
      ...
      const insertCornerBetween = (
        ...
        const updates: Partial<BoardElement> = {
          ...
          elbowRoute:
            element.points.length === 3 ? element.elbowRoute : undefined,
        };
  ...
  const renderSelectionBox = () => {
    ...
    const localRotateHandle: Point | null =
      localRotateAnchor && localRotateOutward
        ? {
            x:
              localRotateAnchor.x + localRotateOutward.x * rotateHandleDistance,
            y:
              localRotateAnchor.y + localRotateOutward.y * rotateHandleDistance,
          }
        : null;
    ...
    return (
      <g>
        ...
        <rect
          ...
          strokeWidth={2 / zoom}
          strokeDasharray={
            selectedIds.length === 1 ? undefined : `${5 / zoom},${5 / zoom}`
          }
          pointerEvents="none"
        ...
        {selectedElement &&
          ...
          (() => {
            ...
            const duplicateOffset = 100; // Distance for duplicated shape
            ...
            const filteredEdgeHandles = hasRotateHandle
              ? edgeHandles.filter((edge) => edge.position !== rotateHandleSide)
              : edgeHandles;

            return filteredEdgeHandles.map((edge) => {
              // Calculate handle position (outside the frame)
              const localHandlePos = {
                x: edge.localPoint.x + edge.outward.x * arrowHandleDistance,
                y: edge.localPoint.y + edge.outward.y * arrowHandleDistance,
              };

              // Apply rotation if element is rotated
              const handlePos = rotationDeg
                ? rotatePoint(localHandlePos, center, rotationDeg)
                : localHandlePos;
  ...
  const renderElement = (element: BoardElement, isPreview = false) => {
    ...
    const isDragPreviewElement =
      ...
      originalElements.length === 1 &&
      originalElements[0].id === element.id;
    const dragPreviewKey = isDragPreviewElement
      ? `${lastMousePos.x}:${lastMousePos.y}:${snapTarget?.elementId ?? ""}:${snapTarget?.point?.x ?? ""}:${snapTarget?.point?.y ?? ""}:${snapTarget?.outOfLineOfSight ? "1" : "0"}`
      : "";
    ...
    return (
      <MemoizedElementRenderer
        ...
        isRemotelyEditing={isRemotelyEditing}
        dragPreviewKey={dragPreviewKey}
        zoom={zoom}
        handDrawnMode={handDrawnMode}
        connectorStyle={connectorStyle}
        renderElementInner={renderElementInner}
      />
    );
  };

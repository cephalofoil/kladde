...
/**
 * Adapts a stroke/fill color to be visible on the viewer's current theme.
 * When collaborating across different themes (one user in dark mode, another in light mode),
 * black strokes become invisible on dark backgrounds and white strokes on light backgrounds.
 * This function swaps pure black/white colors to ensure visibility.
 *
 * @param color - The original color from the element
 * @param viewerTheme - The current viewer's theme ("dark" or "light")
 * @returns The adapted color that will be visible on the viewer's theme
 */
export function adaptColorForTheme(
  ...
  // Handle pure black - invisible on dark mode
  if (
    normalizedColor === "#000000" ||
    normalizedColor === "#000" ||
    normalizedColor === "black"
  ) {
    return viewerTheme === "dark" ? "#ffffff" : "#000000";
  }

  // Handle pure white - invisible on light mode
  if (
    normalizedColor === "#ffffff" ||
    normalizedColor === "#fff" ||
    normalizedColor === "white"
  ) {
    return viewerTheme === "light" ? "#000000" : "#ffffff";
  }

  // For near-black colors (very dark grays), also adapt for visibility
  ...
  if (hex.length === 6) {
    ...
    // Very dark colors (luminance < 0.05) - nearly black
    const luminance = (0.299 * r + 0.587 * g + 0.114 * b) / 255;
    if (luminance < 0.05 && viewerTheme === "dark") {
      return "#ffffff";
    }

    // Very light colors (luminance > 0.95) - nearly white
    if (luminance > 0.95 && viewerTheme === "light") {
      return "#000000";
    }
  }

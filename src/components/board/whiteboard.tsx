...
interface WhiteboardProps {
  boardId: string;
  /** When true, this is a guest joining via collab link */
  collabMode?: boolean;
  /** Pre-imported encryption key for hash-based collab URLs */
  collabEncryptionKey?: CryptoKey;
  /** View-only mode from hash URL */
...
export function Whiteboard({
  ...
  // Check for collaboration mode via ?collab=1 parameter OR prop
  const isCollabModeFromUrl = searchParams?.get("collab") === "1";
  const isCollabMode = collabMode ?? isCollabModeFromUrl;
  ...
  const [collabBoardName, setCollabBoardName] = useState<string | null>(null);
  ...
  const [exportedEncryptionKey, setExportedEncryptionKey] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    ...
    const initializeCollaboration = async () => {
      ...
      // Get encryption key - either from prop (hash-based URL) or from URL hash (legacy)
      let encryptionKey: CryptoKey | undefined;
      if (collabEncryptionKey) {
        // Key provided via prop from hash-based collab URL
        encryptionKey = collabEncryptionKey;
        console.log("[Whiteboard] E2E encryption enabled (from hash URL)");
      } else if (isEncryptionSupported()) {
        const keyString = getKeyFromUrl();
        if (keyString) {
          // Legacy: Guest joining via old URL format with key in hash
          try {
            encryptionKey = await importKeyFromString(keyString);
            console.log(
              "[Whiteboard] E2E encryption enabled (joined with key)",
            );
          } catch (error) {
            console.error(
          ...
          // No key in URL - room is not encrypted yet
          // Key will be generated when owner opens the share dialog
          console.log(
            "[Whiteboard] No encryption key in URL, encryption will be enabled when sharing",
          );
        }
      }
        ...
        setRemoteSelections((prev) => {
          ...
          const changed = selections.some(
            (s, i) =>
              ...
              s.elementIds.length !== prev[i]?.elementIds.length ||
              s.elementIds.some((id, j) => id !== prev[i]?.elementIds[j]),
          );
  ...
  useEffect(() => {
    ...
    const applyMetadata = (metadata: { name?: string }) => {
      const nextName = metadata.name?.trim();
      setCollabBoardName(nextName || null);
    };

    applyMetadata(collaboration.getMetadata());

    const unsubscribe = collaboration.onMetadataChange((metadata) => {
  ...
  useEffect(() => {
    ...
    const currentName = collaboration.getMetadata().name;
    if (currentName !== trimmedName) {
      collaboration.setMetadata({ name: trimmedName });
    }
  }, [collaboration, isOwner, boardName]);
  ...
  const handleAddElement = useCallback(
    ...
    [clearHistoryPreview, collaboration, elements, saveToUndoStack, isReadOnly],
  );
  ...
  const handleUpdateElement = useCallback(
    (id: string, updates: Partial<BoardElement>) => {
      clearHistoryPreview();
      if (isReadOnly) return;
      const currentElement = elementsRef.current.find((el) => el.id === id);
      const nextUpdates =
  ...
  const resolveCommentPosition = useCallback(
    (comment: BoardComment) => {
      if (comment.elementId) {
        ...
        if (box) {
          const offsetX = comment.offset?.x ?? Math.max(0, box.width / 2);
          const offsetY = comment.offset?.y ?? Math.max(0, box.height / 2);
  ...
  const handleAddCommentMessage = useCallback(
    (commentId: string, text: string) => {
      ...
      if (collaboration) {
        ...
        setComments(
          comments.map((item) =>
            item.id === commentId ? { ...item, messages: nextMessages } : item,
          ),
        );
      }
    },
    ...
    (commentId: string, messageId: string, emoji: string) => {
      ...
      const nextMessages = (comment.messages || []).map((message) => {
        ...
        if (existing) {
          ...
          nextReactions = [...reactions, { emoji, userIds: [userId] }];
        }
  ...
  const handleToggleCommentResolved = useCallback(
    (commentId: string) => {
      ...
      if (collaboration) {
        ...
        setComments(
          comments.map((item) =>
            item.id === commentId ? { ...item, resolved: nextResolved } : item,
          ),
        );
      }
    },
  ...
  const markCommentsSeen = useCallback(() => {
    ...
    if (typeof window !== "undefined") {
      localStorage.setItem(`kladde-comments-seen-${boardId}`, String(now));
    }
  }, [boardId]);
  ...
  const handleRestoreHistory = useCallback(
    ...
    [clearHistoryPreview, isOwner, collaboration, setElements, saveToUndoStack],
  );
  ...
  const handleEndTransform = useCallback(() => {
    if (!isTransformingRef.current || !historyManagerRef.current || !isOwner) {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Alt+R: Toggle view mode (works in all modes including read-only)
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden flex">
      ...
      <div className="relative flex-1 overflow-hidden">
        {/* Burger Menu - Top Left (always visible) */}
        ...
        {/* Title Bar - Top Left (hidden in view mode) */}
        <div
          className={cn(
            "absolute top-4 left-16 z-50 flex items-center gap-2 transition-all duration-300",
            viewMode && "opacity-0 -translate-y-4 pointer-events-none",
          )}
        >
          <CanvasTitleBar
            boardId={boardId}
        ...
        {/* Collaboration + Hotkeys - Top Right (hidden in view mode) */}
        {!isReadOnly && (
          <div
            className={cn(
              "absolute top-4 right-4 z-50 flex items-stretch gap-2 transition-all duration-300",
              viewMode && "opacity-0 -translate-y-4 pointer-events-none",
            )}
          >
            <CollaborationBar
        ...
        {!isOwner && (
          <Dialog open={showOwnerDisconnectModal} onOpenChange={() => {}}>
            <DialogContent className="max-w-md [&_[data-dialog-close]]:hidden">
              <DialogHeader>
                <DialogTitle>
                  {ownerSessionEnded ? "Session ended" : "Owner disconnected"}
                </DialogTitle>
                <DialogDescription>
                  {ownerSessionEnded
        ...
        <Canvas
          ...
          onSelectComment={(commentId) => setSelectedCommentIds([commentId])}
